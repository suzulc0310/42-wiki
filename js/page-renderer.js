// Основной рендерер страницы
const PageRenderer = {
    // Текущие данные персонажа
    currentData: null,
    currentStyle: 'Дефолт', // Добавляем текущий стиль
    
    // Инициализация
    async init() {
        // Получаем имя персонажа из URL
        const urlParams = new URLSearchParams(window.location.search);
        const characterName = urlParams.get('name');
        
        if (!characterName) {
            this.showError('Персонаж не указан');
            return;
        }
    ...
    async loadCharacter(characterId) {
        try {
            ...
            // Устанавливаем текущий стиль из localStorage или первый стиль
    ...
    renderAll() {
        ...
        // После рендера контента создаем оглавление
        this.renderTableOfContents();
         setTimeout(() => {
            if (window.imageModalInstance) {
                window.imageModalInstance.refresh();
            }
        }, 100); // Добавляем расчет возраста
        this.calculateAge();
    },
    
    // НОВАЯ ФУНКЦИЯ: Расчет возраста
    calculateAge() {
        document.querySelectorAll('.aside_info').forEach(i => {
            let h = i.querySelector('h3');
            let p = i.querySelector('p');
            
            // Проверяем, что это поле с датой рождения и там еще нет возраста
    ...
    // Заголовок
    renderHeader() {
        document.getElementById('character-name').textContent = this.currentData.title;
    ...
    switchStyle(styleName) {
        ...
        // Обновляем изображения в галерее стилей
        ...
        // Отправляем событие о смене стиля
    ...
    // Показать изображения для стиля
    showImagesForStyle(styleName) {
        ...
        console.log('Показ изображений для стиля:', styleName);
    ...
    renderStyleGallery() {
        ...
        // Перебираем все стили и их изображения
        Object.entries(this.currentData.images).forEach(([styleName, imageUrls]) => {
            imageUrls.forEach(src => {
                ...
                // Показываем только изображения текущего стиля
    ...
    Object.entries(data.required || {}).forEach(([key, value]) => {
        ...
        // ВОТ ЗДЕСЬ ДОБАВЛЯЕМ ПОДДЕРЖКУ МАССИВА ГОРОДОВ
            ...
            switch(section.type) {
                ...
                case 'text-with-list':
                    if (section.content.textBefore) {
                        html += `<p>${section.content.textBefore}</p>`;
                    }
    ...
    // Оглавление с поддержкой подзаголовков
        ...
        document.body.innerHTML = `
            <div class="err">
                <div class="err_center white">
                    <h1 class="err_heading">ОШИБКА 442</h1>
                    <h1>Страницу украла крыса</h1>
                    <p>Пожалуйста, подождите обновлений, а пока вернитесь назад</p>
                    <p>(Нажмите на крыску)</p>
                    <a id="elem" href="javascript:window.history.back();">
                    <svg width="150" viewBox="0 0 72 72" id="emoji" xmlns="http://www.w3.org/2000/svg" fill="#000000">
                    <g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <